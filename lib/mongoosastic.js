...
module.exports = function Mongoosastic(schema, options) {
    ...
    schema.statics.synchronize = function (query) {
        ...
        stream.on('data', function (doc) {
            visible++;
            var pending = visible - completed;
            if (pending > 5 * bulk.size && !stream.isPaused()) {
                stream.pause();
            } else if (pending < bulk.size && stream.isPaused()) {
                stream.resume();
            }
    ...
    function bulkIndex(options, cb) {
        ...
        if (options.routing) {
            finalParam.update.routing = options.routing;
        }
    ...
    schema.statics.flush = function (cb) {
        ...
        var thisBulkBuffer = bulkBuffer;
        var thisBulkCallbacks = bulkCallbacks;
        ...
        esClient.bulk({
            ...
            return _cb(_err, results, thisBulkBuffer, thisBulkCallbacks);
    ...
    function setUpMiddlewareHooks(schema) {
        ...
        var queryHookFunc = function (result) {
            if ((typeof this._conditions == 'object') && (Object.getOwnPropertyNames(this._conditions).length > 0)) {
                ...
                this.model.find(this._conditions, function (err, results) {
                    if (err) {
                        return
                    }
